...
export class ConfigurationsService {
  ...
  get(filter: FilterList): Observable<ListPage> {
   ...
   return this.http.get<ListPage>(UriConstante.CONFIGURATIONS_RESOURCE + "/", { params: params });
  }
  getById(id: number): Observable<Configurations> {
    return this.http.get<Configurations>(UriConstante.CONFIGURATIONS_RESOURCE + `/${id}`);
  }
  register(data: Configurations): Observable<ResponseMessage> {
    return this.http.post<ResponseMessage>(UriConstante.CONFIGURATIONS_RESOURCE + "/", data);
  }
  ...
  updateStatus(id: number, status: string): Observable<ResponseMessage> {
    const body = { status };
    return this.http.patch<ResponseMessage>( `${UriConstante.CONFIGURATIONS_RESOURCE}/change-status/${id}`, body);
  }
  deleteConfig(id: number): Observable<ResponseMessage> {
    return this.http.delete<ResponseMessage>( `${UriConstante.CONFIGURATIONS_RESOURCE}/${id}`);
  }
}
