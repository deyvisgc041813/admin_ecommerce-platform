...
import { Category, ListPage, ResponseMessage, SubCategory, UriConstante } from '../..';
...
export class SubcategoryService {
  ...
  getById(id: number): Observable<SubCategory> {
    return this.http.get<SubCategory>(UriConstante.SUBCATEGORY_RESOURCE + `/${id}`);
  }
  register(category: SubCategory[]): Observable<ResponseMessage> {
    return this.http.post<ResponseMessage>(UriConstante.SUBCATEGORY_RESOURCE + "/", category);
  }
  ...
  delete(id: number) {
    return this.http.delete<ResponseMessage>(UriConstante.SUBCATEGORY_RESOURCE + `/${id}`);
  }
  ...
  getSubcategoriesWithCategories (): Observable<ListPage> {
    return this.http.get<ListPage>(UriConstante.SUBCATEGORY_RESOURCE + "/subcategories-categories");
  }
}
